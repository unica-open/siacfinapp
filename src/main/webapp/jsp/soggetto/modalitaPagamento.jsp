		...
		<div class="row-fluid">
			<div class="span12 ">
				...
				<div class="contentPage">
					...
					<s:form method="post" action="modalitaPagamentoSoggetto">
						...
						<ul class="nav nav-tabs">
							...
							<s:else>
								<!-- task-131 <li><s:a action="aggiornaSoggetto" method="doExecute">Soggetto</s:a></li>-->
								<!-- task-131 <li><s:a action="aggiornaSediSecondarie" method="doExecute">Sedi Secondarie</s:a></li> -->
								<li><s:a action="aggiornaSoggetto_doExecute">Soggetto</s:a></li>
								<li><s:a action="aggiornaSediSecondarie_doExecute">Sedi Secondarie</s:a></li>
								<li class="active"><a href="#">Modalit&agrave;
										pagamento</a></li>
							</s:else>
						</ul>
						...
						<h3>
							Codice Soggetto:
							<s:property value="dettaglioSoggetto.codiceSoggetto" />
							-
							<s:property value="dettaglioSoggetto.denominazione" />
							(
							<s:property value="dettaglioSoggetto.statoOperativo" />
							dal
							<s:property
								value="%{dettaglioSoggetto.dataValidita}" />
							)
						</h3>
						...
						%>
						<display:table
							name="model.dettaglioSoggetto.modalitaPagamentoList"
							class="table tab_left table-hover"
							summary="riepilogo modalita pagamento"
							requestURI="modalitaPagamentoSoggetto.do" uid="modPag">
							...
							<display:column title="Descrizione">
								...
								<s:push value="%{#attr.modPag}">
									<s:include
										value="/jsp/include/descrizioneCompletaModalitaPagamento.jsp" />
								</s:push>
							...
							<display:column
								title="<abbr title='progressivo'>Associato a</abbr>"
								property="associatoA" />
							<display:column title="Stato"
								property="descrizioneStatoModalitaPagamento" />
							<display:column title="" class="tab_Right">
								<div class="btn-group">
									...
									<ul class="dropdown-menu pull-right"
										id="ul_action_<s:property value="%{#attr.ricercaSoggettoID.codiceSoggetto}"/>">
										<s:if test="isAbilitato(1, #attr.modPag.uid)">
											<li><a href="#consultaModPag_<%=indice%>"
												data-toggle="modal">consulta</a></li>
										</s:if>
										<s:if test="isAbilitato(2, #attr.modPag.uid)">
											<li><a
												href="modalitaPagamentoSoggetto_aggiornaMDP.do?aggiornaCodiceSoggetto=${modPag.uid}"
												data-toggle="modal">aggiorna</a></li>
										</s:if>
										<s:if test="isAbilitato(3, #attr.modPag.uid)">
											<li><a href="#msgAnnulla"
												onclick="$('.annulla-prosegui').attr('href', 'modalitaPagamentoSoggetto_annullaModalitaDiPagamento.do?aggiornaCodiceSoggetto=' + ${modPag.uid})"
												data-toggle="modal">annulla</a></li>
										</s:if>
										<s:if test="isAbilitato(5, #attr.modPag.uid)">
											<li><a href="#msgBlocca"
												onclick="$('.blocca-prosegui').attr('href', 'modalitaPagamentoSoggetto_bloccaModalitaDiPagamento.do?aggiornaCodiceSoggetto=' + ${modPag.uid})"
												data-toggle="modal">blocca</a></li>
										</s:if>
										<s:if test="isAbilitato(6, #attr.modPag.uid)">
											<li><a href="#msgElimina"
												onclick="$('.elimina-prosegui').attr('href', 'modalitaPagamentoSoggetto_eliminaMdp.do?aggiornaCodiceSoggetto=' + ${modPag.uid})"
												data-toggle="modal">elimina</a></li>
										</s:if>
										<s:if test="isAbilitato(7, #attr.modPag.uid)">
											<s:if test="%{#attr.modPag.inModifica}">
												<li><a
													href="validaModalitaPagamento.do?mdpId=${modPag.uidOrigine}"
													data-toggle="modal">valida</a></li>
											</s:if>
											<s:else>
												<li><a
													href="validaModalitaPagamento.do?mdpId=${modPag.uid}"
													data-toggle="modal">valida</a></li>
											</s:else>
						...
						<s:if test="%{azioneAggiorna == false}">
							<p style="margin-bottom: 10px;" class="margin-medium">
								<a class="btn"
									href="modalitaPagamentoSoggetto_panelInsertPayment.do"
									data-toggle="collapse" data-target="#inseriscipag">inserisci
									nuove modalit&agrave;</a>
							</p>
						</s:if>
							...
							<div id="inseriscipag">
								...
								<div class="accordion_info">
									<fieldset class="form-horizontal">
										<s:form name="handlePaymentType" action="modalitaPagamentoSoggetto" method="post">
											<div class="control-group">
												<label class="control-label" for="tipoind">Associato
													a *</label>
												<div class="controls">
													<s:if test="null!=modelWeb.associataList">
														<s:select list="modelWeb.associataList" id="tipoind"
															name="modelWeb.tipoind" />
													</s:if>
												</div>
											</div>

											<div class="control-group">
												<label class="control-label" for="tipoind">Tipo
													accredito *</label>
												<div class="controls">
													<s:if test="null!=listaTipoAccredito">
														<s:select list="listaTipoAccredito"
															id="idAccreditoTipoSelected"
															name="modelWeb.idAccreditoTipoSelected" listKey="id"
															listValue="%{codice + ' - ' + descrizione}"
															headerKey="-1" headerValue="Scegli il tipo di accredito"
															title="Scegli il tipo di accredito"
...
.hideButton {
	display: none !important;
											...
											<s:submit id="submitPaymentType" name="handle_payment_type" value="seleziona tipologia" action="modalitaPagamentoSoggetto_handleTypePayment" cssClass="hideButton" />
 ...
 -->
         ...
         <a class="btn"  data-toggle="collapse" data-target="#insInd">inserisci nuove modalit&agrave;</a>
						...
						<s:if test="%{contoCorrenteVisible}">
							<s:form name="insertModPagContoCorrente" action="modalitaPagamentoSoggetto" method="post">
								<s:hidden name="tipoAccreditoPage" id="tipoAccreditoPage"
									value="tipoAccreditoPage" />
								<div id="inscontocorr">
									<a id="ancorettaMdp"></a>
									<div class="accordion_info">
										<fieldset class="form-horizontal">
											<s:if test="%{modelWeb.codeMdpSelected != 'CBI'}">
												<div class="control-group">
													...
													<div class="controls">
														<s:textfield id="ibanToInsert"
															name="modalitaPagamentoSoggettoToInsert.iban"
											...
											<s:if
												test="%{modelWeb.codeMdpSelected != 'CCP' && modelWeb.codeMdpSelected != 'CBI' }">
												<div class="control-group">
													<label class="control-label" for="bic">BIC </label>
													<div class="controls">
														<s:textfield id="bicToInsert"
															name="modalitaPagamentoSoggettoToInsert.bic"
															cssClass="span3 required" />
													</div>
												</div>
											</s:if>
											...
											<div class="control-group">
												<label class="control-label" for="conto">Numero
													conto</label>
												<div class="controls">
													<s:textfield id="ncontoCorrenteToInsert"
														name="modalitaPagamentoSoggettoToInsert.contoCorrente"
														cssClass="span3 required" />
												</div>
											</div>

											<div class="control-group">
												<label class="control-label" for="denominazioneBanca">Denominazione
													banca</label>
												<div class="controls">
											...
											<div class="control-group">
												<label class="control-label" for="intconto">Intestazione
													conto</label>
												<div class="controls">
													<s:textfield id="nIntestazioneToInsert"
														name="modalitaPagamentoSoggettoToInsert.intestazioneConto"
														maxlength="499" cssClass="span3 required" />
												</div>
											</div>

											<div class="control-group">
												<label class="control-label" for="scadenza">Data
													cessazione</label>
												<div class="controls">
											...
											<div class="control-group">
												...
												<div class="controls">
													<s:textarea id="notaToInsert"
														name="modalitaPagamentoSoggettoToInsert.note"
														cssClass="span3" />
												</div>
											</div>

											<div class="control-group">
												<label class="control-label" for="notee">Per gli stipendi</label>
												<div class="controls">
													<s:checkbox id="perStipendiToInsert"
														name="modalitaPagamentoSoggettoToInsert.perStipendi"
														cssClass="" />
												</div>
											</div>
										...
										<p>
											<a class="btn"
												href="modalitaPagamentoSoggetto_annullaInserimento.do">annulla
												inserimento</a>
											<!--task-131 <s:submit name="inserisciModPagCorrente" value="Salva" id="salvaId" cssClass="btn" method="inserisciModPagContoCorrente" /> -->
											<s:submit name="inserisciModPagCorrente" value="Salva" id="salvaId" cssClass="btn" action="modalitaPagamentoSoggetto_inserisciModPagContoCorrente" />
										</p>
									</div>
								</div>
							</s:form>
						...
						<s:if test="%{genericoVisibile}">
							<s:form name="insertModPagGenerico" action="modalitaPagamentoSoggetto" method="post">
								<s:hidden name="tipoAccreditoPage" id="tipoAccreditoPage"
									value="tipoAccreditoPage" />
								<div id="ins-generico">
									<a id="ancorettaMdp"></a>
									<div class="accordion_info">
										...
										<p>
											<a class="btn" href="modalitaPagamentoSoggetto_annullaInserimento.do">annulla inserimento</a>
											<!-- task-131 <s:submit name="inserisciModPagGenerico" value="Salva" id="salvaId" cssClass="btn" method="inserisciModPagGenerico" /> -->
											<s:submit name="inserisciModPagGenerico" value="Salva" id="salvaId" cssClass="btn" action="modalitaPagamentoSoggetto_inserisciModPagGenerico" />
										</p>
									</div>
								</div>
							</s:form>
						...
						<s:if test="%{contanteVisible}">
							<s:form name="inserisciModPagContante" action="modalitaPagamentoSoggetto" method="post">
								<s:hidden name="tipoAccreditoPage" id="tipoAccreditoPage"
									value="tipoAccreditoPage" />
								<div id="inscontante">
									<a id="ancorettaMdp"></a>
									<div class="accordion_info">
										<fieldset class="form-horizontal">
											<div class="control-group">
												<label class="control-label" for="nomevia">Codice
													fiscale Quietanzante</label>
												<div class="controls">
													<s:textfield id="codFiscale"
														name="modalitaPagamentoSoggettoToInsert.codiceFiscaleQuietanzante"
														cssClass="span3" />
												</div>
											</div>
											...
											<div class="control-group">
												...
												<div class="controls">
													<s:textfield id="quietanzante"
														name="modalitaPagamentoSoggettoToInsert.soggettoQuietanzante"
														cssClass="span3" />
												</div>
											</div>

											<div class="control-group">
												<label class="control-label" for="dataNascita">Data
													di nascita quietanzante</label>
												<div class="controls">
													<s:textfield id="dataNascitaToInsert"
														name="modalitaPagamentoSoggettoToInsert.dataNascitaQuietanzante"
														maxlength="10" cssClass="span3" />
												</div>
											</div>
											...
											<div class="control-group">
												<label class="control-label" for="Stato2">Luogo
													nascita quietanzante</label>
												<div class="controls">

													<s:select list="nazioni" id="idNazione" headerKey=""
														headerValue=""
														name="modalitaPagamentoSoggettoToInsert.comuneNascita.nazioneCode"
														listKey="codice" listValue="descrizione" />

													<span class="al"> </span>
													<s:textfield id="comune"
														name="modalitaPagamentoSoggettoToInsert.comuneNascita.descrizione"
														cssClass="lbTextSmall span3" />
													<s:hidden id="comuneId"
														name="modalitaPagamentoSoggettoToInsert.comuneNascita.uid"></s:hidden>
										...
										<p>
											...
											<s:submit cssClass="btn" value="Salva" id="salvaId" action="modalitaPagamentoSoggetto_inserisciModPagContanti" name="salvaContante" />
										</p>
									</div>
								</div>
							</s:form>
						...
						<s:if test="%{cessioneVisible}">
							<div id="inscessione">
								...
								<div class="accordion_info">
									...
									<s:form id="ricercaSoggettoModPag" action="modalitaPagamentoSoggetto" method="post">
										<fieldset class="form-horizontal">
											<div class="control-group">
												<label class="control-label" for="Codice">Codice</label>
												<div class="controls">
													<s:textfield id="codice" name="modelWeb.codice"
														cssClass="span4" />
													<label for="den" class="radio inline">Denominazione</label>
													<s:textfield id="denominazione"
														name="modelWeb.denominazione" cssClass="span6 required" />
												</div>
											</div>
											<div class="control-group">
												<label class="control-label" for="iva">Partita IVA</label>
												<div class="controls">
													<s:textfield id="partitaIva" name="modelWeb.partitaIva"
														cssClass="span4 required" />
													<label class="radio inline" for="codfisc">Codice
														Fiscale</label>
													<s:textfield id="codiceFiscale"
														name="modelWeb.codiceFiscale" cssClass="span6 required" />
												</div>
											</div>
										</fieldset>
										<p>
											<a class="btn btn-link"
												href="modalitaPagamentoSoggetto_annullaInserimento.do">annulla</a>
											<!-- task-131 <s:submit name="cerca" value="cerca" method="ricercaSoggettoModPag" cssClass="btn" /> -->
											<s:submit name="cerca" value="cerca" action="modalitaPagamentoSoggetto_ricercaSoggettoModPag" cssClass="btn" />
										</p>
									</s:form>

									<s:if test="%{cessioneSearchVisible}">
										<s:form id="selezioneSoggettoCessioneForm" action="modalitaPagamentoSoggetto" method="post">
											<s:hidden name="tipoAccreditoPage" id="tipoAccreditoPage"
												value="tipoAccreditoPage" />
											...
											<table class="table table-hover" summary="riepilogo soggetti">
												...
												<tbody>
													<s:iterator value="modelWeb.soggettiRicercati"
														var="soggettoRicercato">
														<tr>
															<td scope="row"><label
																for="${soggettoRicercato.codiceSoggetto}"> <input
																	value="${soggettoRicercato.codiceSoggetto}"
																	type="radio" name="codiceSoggettoRicercatoCessione"
																	id="${soggettoRicercato.codiceSoggetto}" />
																	${soggettoRicercato.codiceSoggetto}
															</label></td>
											...
											<p>
												<a class="btn btn-link" href="modalitaPagamentoSoggetto_annullaInserimento.do">annulla</a>
												<!-- task-131 <s:submit name="seleziona_soggetto" value="seleziona soggetto" method="ricercaSoggettoModPag_step2" cssClass="btn" /> -->
												<s:submit name="seleziona_soggetto" value="seleziona soggetto" action="modalitaPagamentoSoggetto_ricercaSoggettoModPag_step2" cssClass="btn" />
											</p>
										</s:form>
									</s:if>

									<s:if test="%{cessioneSearchStep2}">
										<s:form name="inserisciModPagCessione" action="modalitaPagamentoSoggetto" method="post">
											<s:hidden name="tipoAccreditoPage" id="tipoAccreditoPage"
												value="tipoAccreditoPage" />
											<h5>Soggetto selezionato:
												${modelWeb.dettaglioSoggettoCessione.denominazione} &nbsp; -
												&nbsp; ${modelWeb.dettaglioSoggettoCessione.codiceFiscale}</h5>
											<a id="ancorettaMdp"></a>
											<fieldset class="form-horizontal">

												<div class="control-group">
													<label class="control-label" for="pagamento">Modalit&agrave;
														di pagamento</label>
													<div class="controls">
														<s:select cssStyle="width: 600px;"
															list="modelWeb.modalitaPagamentoCessione"
															id="modPagCessione"
															name="modalitaPagamentoSoggettoToInsert.uid"
															title="Scegli la modalita di pagamento" listKey="uid"
															listValue="%{'(' + modalitaAccreditoSoggetto.codice + ' - ' + modalitaAccreditoSoggetto.descrizione + ') ' + descrizione}" />
													</div>
												</div>
												<div class="control-group">
													<label class="control-label" for="notee">Note</label>
                ...
                <select id="pagamento" name="pagamento"><option>&nbsp;</option>
											...
											<p>
												...
												<s:submit cssClass="btn" name="salvaCessione" id="salvaId" value="Salva" action="modalitaPagamentoSoggetto_inserisciModPagCessione" />
											</p>
										</s:form>
									</s:if>
								</div>
						...
						<%
							int secondoIndice = 0;
						%>
						<s:iterator value="model.dettaglioSoggetto.modalitaPagamentoList"
							var="pagamento">
							<div id="consultaModPag_<%=secondoIndice%>"
	             ...
	             -->
											<s:if
												...
												<dt style="width: 180px;">Soggetto Ricevente</dt>
	             ...
	             -->
						...
						<s:if test="%{aggiornaContoCorrenteVisible}">
							<s:form name="aggiornaModPagContoCorrente"
								action="modalitaPagamentoSoggetto" method="post">
								<s:hidden name="aggiornaTipoAccredito"
									id="aggiornaTipoAccredito" value="aggiornaTipoAccredito" />
								<div id="inscontocorr">
									<a id="ancorettaMdp"></a>
									<div class="accordion_info">
										<fieldset class="form-horizontal">
											<s:if test="%{aggiornaTipoAccredito != 'CBI'}">
												<div class="control-group">
													...
													<div class="controls">
														<s:textfield id="ibanToInsert"
															name="soggettoDaAggiornare.iban"
															cssClass="span3 required" />
													</div>
												</div>
											</s:if>
											...
											<s:if
												test="%{aggiornaTipoAccredito != 'CCP' && aggiornaTipoAccredito != 'CBI' }">
												<div class="control-group">
													<label class="control-label" for="bic">BIC </label>
													<div class="controls">
														<s:textfield id="bicToInsert"
															name="soggettoDaAggiornare.bic" cssClass="span3 required" />
													</div>
												</div>
											</s:if>
											...
											<div class="control-group">
												...
												<div class="controls">
													<s:textfield id="ncontoCorrenteToInsert"
														name="soggettoDaAggiornare.contoCorrente"
														cssClass="span3 required" />
												</div>
											</div>
											...
											<div class="control-group">
												<label class="control-label" for="intestazioneConto">Intestazione
													conto</label>
												<div class="controls">
													<s:textfield id="nIntestazioneContoCorrenteToInsert"
														name="soggettoDaAggiornare.intestazioneConto"
														cssClass="span3 required" />
												</div>
											</div>
											...
											<div class="control-group">
												...
												<div class="controls">
													<s:textarea id="notaToInsert"
														name="soggettoDaAggiornare.note" cssClass="span3" />
												</div>
											</div>
											...
											<div class="control-group">
												...
												<div class="controls">
													<s:checkbox id="perStipendiToInsert"
														name="soggettoDaAggiornare.perStipendi"
														cssClass="" />
												</div>
											</div>
										...
										<p>
											...
											<s:submit name="aggiornaModPagCorrente" value="Salva" id="aggiornaId" cssClass="btn" action="modalitaPagamentoSoggetto_aggiornaModPagContoCorrente" />
										</p>
									</div>
								</div>
							</s:form>
						...
						<s:if test="%{aggiornaGenericoVisible}">
							<s:form name="aggiornaModPagGenerico"
								action="modalitaPagamentoSoggetto" method="post">
								<s:hidden name="aggiornaTipoAccredito"
									id="aggiornaTipoAccredito" value="aggiornaTipoAccredito" />
								<div id="agg-generico">
									...
									<div class="accordion_info">
										...
										<p>
											...
											<s:submit name="aggiornaGenerico" value="Salva" id="aggiornaId" cssClass="btn" action="modalitaPagamentoSoggetto_aggiornaGenerico" />
										</p>
									</div>
								</div>
							</s:form>
						...
						<s:if test="%{aggiornaContantiVisible}">
							<s:form name="aggiornaModPagContante"
								action="modalitaPagamentoSoggetto" method="post">
								<s:hidden name="aggiornaTipoAccredito"
									id="aggiornaTipoAccredito" value="aggiornaTipoAccredito" />
								<div id="inscontante">
									...
									<div class="accordion_info">
										<fieldset class="form-horizontal">
											<div class="control-group">
												...
												<div class="controls">
													<s:textfield id="codFiscale"
														name="soggettoDaAggiornare.codiceFiscaleQuietanzante"
														cssClass="span3" />
												</div>
											</div>
											...
											<div class="control-group">
												...
												<div class="controls">
													<s:textfield id="quietanzante"
														name="soggettoDaAggiornare.soggettoQuietanzante"
														cssClass="span3" />
												</div>
											</div>

											<div class="control-group">
												<label class="control-label" for="scadenza">Data di
													nascita quietanzante</label>
												<div class="controls">
													<s:textfield id="nascitaToInsert"
														name="soggettoDaAggiornare.dataNascitaQuietanzante"
														maxlength="10" cssClass="span3" />
												</div>
											</div>

											<div class="control-group">
												<label class="control-label" for="luogoNascita">Luogo
													nascita quietanzante</label>
												<div class="controls">

													<s:select list="nazioni" id="idNazione" headerKey=""
														headerValue=""
														name="soggettoDaAggiornare.comuneNascita.nazioneCode"
														listKey="codice" listValue="descrizione" />

													<span class="al"> </span>
													<s:textfield id="comune"
														name="soggettoDaAggiornare.comuneNascita.descrizione"
														cssClass="lbTextSmall span3" />
													<s:hidden id="comuneId"
														name="soggettoDaAggiornare.comuneNascita.uid"></s:hidden>
												</div>
											</div>
											...
											<div class="control-group">
												<label class="control-label" for="notee">Note</label>
												<div class="controls">
													<s:textarea id="notee" name="soggettoDaAggiornare.note"
														cssClass="span3" />
												</div>
											</div>
										...
										<p>
											...
											<s:submit cssClass="btn" value="Salva" id="aggiornaId"	action="modalitaPagamentoSoggetto_aggiornaContante" name="aggiornaContante" />
										</p>
									</div>
								</div>
							</s:form>
						...
						<s:if test="%{aggiornaCessioneSearch}">
							<div id="inscessione">
								...
								<div class="accordion_info">
									<h5>Cerca Soggetto</h5>
									<s:form id="aggiornaricercaSoggettoModPag"
										action="modalitaPagamentoSoggetto" method="post">
										<s:hidden name="aggiornaTipoAccredito"
											id="aggiornaTipoAccredito" value="aggiornaTipoAccredito" />
										<fieldset class="form-horizontal">
											<div class="control-group">
												<label class="control-label" for="Codice">Codice</label>
												<div class="controls">
													<s:textfield id="codice" name="modelWeb.codiceAggiorna"
														cssClass="span4" />
													<label for="den" class="radio inline">Denominazione</label>
													<s:textfield id="denominazione"
														name="modelWeb.denominazioneAggiorna"
											...
											<div class="control-group">
												<label class="control-label" for="iva">Partita IVA</label>
												<div class="controls">
													<s:textfield id="partitaIva"
														name="modelWeb.partitaIvaAggiorna"
														cssClass="span4 required" />
													<label class="radio inline" for="codfisc">Codice
														Fiscale</label>
													<s:textfield id="codiceFiscale"
														name="modelWeb.codiceFiscaleAggiorna"
														cssClass="span6 required" />
												</div>
											</div>
										</fieldset>
										<p>
											...
											<s:submit name="cerca" value="cerca" action="modalitaPagamentoSoggetto_ricercaSoggettoModPagAggiorna" cssClass="btn" />
										</p>
									</s:form>

									<s:if test="%{aggiornaCessioneStep2}">
										<s:form id="selezioneSoggettoCessioneAggiornaForm" action="modalitaPagamentoSoggetto" method="post">
											<s:hidden name="aggiornaTipoAccredito"
												id="aggiornaTipoAccredito" value="aggiornaTipoAccredito" />
											<h5>Risultati della ricerca</h5>
											<table class="table table-hover" summary="riepilogo soggetti">
												...
												<tbody>
													<s:iterator value="modelWeb.soggettiRicercatiAggiorna" var="soggettoRicercatoAggiorna">
														<tr>
															<td scope="row"><label
																for="${soggettoRicercatoAggiorna.codiceSoggetto}">
																	<input
																	value="${soggettoRicercatoAggiorna.codiceSoggetto}"
																	type="radio"
																	name="codiceSoggettoRicercatoCessioneAggiorna"
																	id="${soggettoRicercatoAggiorna.codiceSoggetto}" />
																	${soggettoRicercatoAggiorna.codiceSoggetto}
															</label></td>
											...
											<p>
												...
												<s:submit name="seleziona_soggetto" value="seleziona soggetto" action="modalitaPagamentoSoggetto_ricercaSoggettoModPag_step2Aggiorna" cssClass="btn" />
									...
									<s:if test="%{aggiornaCessioneStep3}">
										<s:form name="inserisciModPagCessione" action="modalitaPagamentoSoggetto" method="post">
											...
											<h5>Soggetto selezionato:
												${modelWeb.dettaglioSoggettoCessioneAggiorna.denominazione}
												&nbsp; - &nbsp;
												${modelWeb.dettaglioSoggettoCessioneAggiorna.codiceFiscale}</h5>
											<a id="ancorettaMdp"></a>
											<fieldset class="form-horizontal">
												...
												<div class="control-group">
													...
													<div class="controls">
														<s:select
															list="modelWeb.modalitaPagamentoCessioneAggiorna"
															id="modPagCessione" name="soggettoDaAggiornare.uid"
															title="Scegli la modalita di pagamento" listKey="uid"
															listValue="modalitaAccreditoSoggetto.descrizione" />
													</div>
												</div>
												<div class="control-group">
													<label class="control-label" for="notee">Note</label>
                ...
                <select id="pagamento" name="pagamento"><option>&nbsp;</option>
											...
											<p>
												...
												<s:submit cssClass="btn" name="salvaCessione" id="aggiornaId" value="" action="modalitaPagamentoSoggetto_aggiornaModPagCessione" />
											</p>
										</s:form>
									</s:if>
								</div>
						...
						<s:if test="%{aggiornaCessioneStep3}">
							<s:form name="inserisciModPagCessione" action="modalitaPagamentoSoggetto" method="post">
								<s:hidden name="aggiornaTipoAccredito" id="aggiornaTipoAccredito" value="aggiornaTipoAccredito" />
								<h5>Codice Soggetto Cessione:
									${soggettoDaAggiornare.cessioneCodSoggetto} - ${soggettoDaAggiornare.soggettoCessione.denominazione}</h5>
								<s:hidden name="soggettoDaAggiornare.cessioneCodSoggetto" />

								<fieldset class="form-horizontal">
									<div class="control-group">
										<label style="width: auto;" class="control-label"
											for="pagamento">Modalit&agrave; di pagamento :
											${soggettoDaAggiornare.modalitaPagamentoSoggettoCessione2.modalitaAccreditoSoggetto.codice}
											-
											${soggettoDaAggiornare.modalitaPagamentoSoggettoCessione2.modalitaAccreditoSoggetto.descrizione}
										</label>
										<s:hidden name="soggettoDaAggiornare.modalitaPagamentoSoggettoCessione2.modalitaAccreditoSoggetto.codice" />
										<s:hidden name="soggettoDaAggiornare.modalitaPagamentoSoggettoCessione2.modalitaAccreditoSoggetto.descrizione" />
									</div>
									<div class="control-group">
										<label class="control-label" for="notee">Note</label>
										<div class="controls">
                ...
                <select id="pagamento" name="pagamento"><option>&nbsp;</option>
								...
								<p>
									...
									<s:submit cssClass="btn" name="salvaCessione" id="aggiornaId" value="Salva" action="modalitaPagamentoSoggetto_aggiornaModPagCessione" />
								</p>
							</s:form>
						</s:if>
						...
						<div id="msgElimina" class="modal hide fade" tabindex="-1"
							...
							<div class="modal-body">
								<div class="alert alert-error">
									...
									<p>Stai per eliminare l'elemento selezionato: sei sicuro di
										voler proseguire?</p>
								</div>
							</div>
							<div class="modal-footer">
								<button class="btn" data-dismiss="modal" aria-hidden="true">no,
									indietro</button>
								<a href="modalitaPagamentoSoggetto_eliminaMdp.do?aggiornaCodiceSoggetto=" class="btn btn-primary elimina-prosegui">si, prosegui</a>
							</div>
						</div>
						<!--/modale elimina -->
						<!-- Modal -->
						<div id="msgAnnulla" class="modal hide fade" tabindex="-1"
							...
							<div class="modal-footer">
								<button class="btn" data-dismiss="modal" aria-hidden="true">no,
									indietro</button>
								<a
									href="modalitaPagamentoSoggetto_annullaModalitaDiPagamento.do?aggiornaCodiceSoggetto="
									class="btn btn-primary annulla-prosegui">si, prosegui</a>
							</div>
						</div>
						<!--/modale annulla -->
						<!-- Modal -->
						<div id="msgBlocca" class="modal hide fade" tabindex="-1"
							role="dialog" aria-labelledby="msgBloccaLabel" aria-hidden="true">
							<div class="modal-body">
								<div class="alert alert-error">
									<button type="button" class="close" data-dismiss="alert">&times;</button>
									<p>
									...
									<p>Stai per bloccare l'elemento selezionato, questo
										cambier&agrave; lo stato dell'elemento: sei sicuro di voler
										proseguire?</p>
								</div>
							</div>
							<div class="modal-footer">
								<button class="btn" data-dismiss="modal" aria-hidden="true">no,
									indietro</button>
								<a
									href="modalitaPagamentoSoggetto_bloccaModalitaDiPagamento.do?aggiornaCodiceSoggetto="
									class="btn btn-primary blocca-prosegui">si, prosegui</a>
							</div>
						</div>
						<!--/modale blocca -->
