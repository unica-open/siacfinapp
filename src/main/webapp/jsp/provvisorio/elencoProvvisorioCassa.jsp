...
<div class="container-fluid">
	<div class="row-fluid">
		<div class="span12 contentPage"> 
				...
				<s:form id="mainForm" method="post" action="elencoProvvisorioCassa.do">  
		...
		<display:table name="elencoProvvisoriCassa" 
			...
			<display:column title="Conto evidenza">
	 		...
	 		<display:column title="Data emissione">
		 		<s:property value="%{#attr.ricercaProvvisorioCassaID.dataEmissione}" /></display:column>	 			 					
			<display:column title="Data annullamento">			 
			 <s:property value="%{#attr.ricercaProvvisorioCassaID.dataAnnullamento}" /></display:column>			 
		...
		<display:column class="sac" title="Struttura amministrativo contabile"> 
	        ...
	        <display:column title="" class="tab_Right">
		    	<div class="btn-group">
		    		...
		    		<ul class="dropdown-menu pull-right" id="ul_action">
		    			...
		    			<s:url id="aggiornaUrl" action="aggiornaProvvisorioCassa.do" escapeAmp="false">
	                    ...
	                    <s:url id="consultaUrl" action="consultaProvvisorioCassa.do" escapeAmp="false">
			        		<s:param name="numeroProvv" value="%{#attr.ricercaProvvisorioCassaID.numero}" />
			        		<s:param name="annoProvv" value="%{#attr.ricercaProvvisorioCassaID.anno}" />
			        		<s:param name="tipoProvv" value="%{#attr.ricercaProvvisorioCassaID.tipoProvvisorioDiCassa}" />
	                    </s:url>
	                    
	                    <%-- condizione vecchia per consulta:
	                    	<s:if test="utenteLettore or (utenteDecentrato and not provvisoriConSacUtente[#attr.ricercaProvvisorioCassaID_rowNum - 1])"> 
	                    --%>
			...
			<s:set var="pageNumParameter" 
          		 value="%{new org.displaytag.util.ParamEncoder('ricercaProvvisorioCassaID').encodeParameterName(@org.displaytag.tags.TableTagParameters@PARAMETER_PAGE)}" />        
  	   		...
  	   		<s:include value="/jsp/include/indietro.jsp" />
  	   			...
  	   			<s:if test="utenteAmministratore">
  	   				<s:if test="not elencoProvvisoriCassa.empty">
  	   		  	   					  <s:submit cssClass="btn btn-primary pull-right" method="aggiornaSac" id="aggiorna-sac" value="aggiorna SAC" />
  	   			         </s:if>  
  	   			     </s:if>  
