...
<div id="refreshtable">	
	...
	<s:if test="isnSubImpegno || hasImpegnoSelezionatoPopup">
		Impegno <s:property value="impegnoPopup.annoMovimento"/>/<s:property value="impegnoPopup.numero.intValue()"/> - 
		<s:if test="impegnoPopup.descrizione!=null && impegnoPopup.descrizione!=''">
			<s:property value="impegnoPopup.descrizione"/> -
		</s:if> 
		<s:property value="getText('struts.money.format', {impegnoPopup.importoAttuale})"/> (${impegnoPopup.descrizioneStatoOperativoMovimentoGestioneSpesa} dal 
		<s:property value="%{impegnoPopup.dataStatoOperativoMovimentoGestioneSpesa}" />)
	</s:if>
